...
function sweetAlertCtrl($scope, SweetAlert) {
	...
	$scope.sucessEquipSave = function (){
		SweetAlert.swal({
			title: "Sucess",
			text: "El equipo se registro con exito"
		});
	}
...
function EquiposCtrl($scope, $http, $state, $rootScope) {
    ...
    $http.get('views/PhpApi/getCategorias.php').success(function (data) {
        $scope.data = {
            Categorias: data,
           catego:{categoriaID:1}
        }
    });  
    $http.get('views/PhpApi/getSucursales.php').success(function (dataSucursal) {
        $scope.dataSucursal = {
	 ...
	 $scope.update = function (id) {
		 if(id== undefined){
			...
			$http.get('views/PhpApi/getSubcategorias.php/?idCate='+id).success(function (data2) {
				$scope.data2 = {
					Subcategorias: data2,
		...
		$scope.getAllEquipoVista =function(){
			$http.get('views/PhpApi/getAllEquipos.php').success(function (dataAllEq) {
				console.log(JSON.stringify(dataAllEq))
				$scope.dataAllEq = {
					equipo: dataAllEq
				}
			});
		  ...
		  //Obtener los equipos por el id de su subcategoria
		$scope.getEquipobySubcat = function () {
			var sucur = $scope.dataSucursal.sucursalMod;
			var categ = $scope.data.catego.categoriaID;
			var subcateg=$scope.data2.subca;
			if(sucur == undefined){
				...
				var params="nomSucursal="+sucur.nombre+
					   "&idcategoria="+categ + 
					   "&idsubcategoria="+subcateg;   
				$http.get('views/PhpApi/getAllEquipos.php/?'+params).success(function (dataAllEq) {
					$scope.dataAllEq = {
						equipo: dataAllEq
					}
				});
	...
	$scope.testSaveE = function (){
		var adl = "Lavadora2";
		var dataN = new FormData();
		dataN.append('archivo',adl);
		
		var saveEq = $http(
               {
                   method: 'post',
                   data: (dataN),
                   url: 'views/PhpApi/equipoAdd.php',
    ...
    function getAllCategoryFTask() {
        $http.get('views/PhpApi/getCategorias.php').success(function (dataCateTask) {
            $scope.dataCateTask = {
    ...
    $scope.updateForTask = function (id) {
        if (id == undefined ) {
            ...
            $http.get('views/PhpApi/getSubcategorias.php/?idCate='+id).success(function (dataSubcaFtask) {
                $scope.dataSubcaFtask = {
    ...
    function getAllChecklist() {
        $http.get("http://localhost:49915/api/checkLists/SelectAll").success(function (datachkList) {
            // console.log(JSON.stringify(datachkList));
            $rootScope.chklist = datachkList;
        });
    }
    $scope.getlastChIDIns = function()  {
        getAllChecklist();
        setTimeout(function () {
            var last = JSON.stringify($rootScope.chklist.reverse()[0]);
	...
	$scope.getAllTaskForOnecheck=function(id){
		alert(id);
    ...
    $scope.saveChecklist = function () {
        if ($scope.periodoMantenimiento.periodomant && $scope.dataCateTask.category && $scope.dataSubcaFtask.subcategory != null) {
            var categoriatask = $scope.dataCateTask.category.categoriaID;
            var subcategoriatask = $scope.dataSubcaFtask.subcategory.subcategoriaID;
            var periodoMan = $scope.periodoMantenimiento.periodomant.peridoMantenimiento;
            alert("periodo mante: " + periodoMan + "Categoria: " + categoriatask + "Subcategoria: " + subcategoriatask);

            var newChecklist =
                     "idEquipo=" + subcategoriatask +
                     "&descripcion=Descripcionpruebaequipo" +
                     "&periodoServicio=" + periodoMan;
			
			var data = new FormData();
	
			data.append('idEquipo',subcategoriatask);
			data.append('descripcion',"ksmkmkmkm");
			data.append('periodoServicio',periodoMan);
			
			var url = "views/PhpApi/checkListAdd.php";
			$.ajax({
			url:url,
			type:'POST',
			contentType:false,
			...
			cache:false,
				success: function (data) {	
					alert(data),
					$scope.getAllTaskForOnecheck(data);},
				error: function (r) { 
						  alert("Error del servidor");
					  }
			});
            ...
            if ($scope.dataCateTask.category == null || $scope.dataCateTask.category == undefined) { faltaCate = "Categoria";}
            if ($scope.dataSubcaFtask == null || $scope.dataSubcaFtask == undefined) { faltaSubcate = "Subcategoria"; }
            else if ($scope.dataSubcaFtask.subcategory == null || $scope.dataSubcaFtask.subcategory == undefined) {  faltaSubcate = "Subcategoria";}
    ...
    getAllCategoryFTask();
    function getAllTaks() {
       $http.get("views/PhpApi/getTareasAll.php").success(function (dataTask) {
           //console.log(JSON.stringify(dataTask));
           $rootScope.tareasck = dataTask;
        });
   }
    getAllTaks();
